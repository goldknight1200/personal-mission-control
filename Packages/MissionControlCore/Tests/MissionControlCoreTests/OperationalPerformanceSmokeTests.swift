...
final class OperationalPerformanceSmokeTests: XCTestCase {
    func testTypicalWeekRepeatedPlanningIsStableAndBounded() {
        ...
        for _ in 0..<25 {
            ...
            XCTAssertEqual(
                Set(result.blocks.map(\.id)).count,
                result.blocks.count
            )
    ...
    func testFiveHundredItemBacklogRemainsBounded() {
        let referenceDate = Date(timeIntervalSince1970: 1_800_000_000)
        var snapshot = MissionControlSeed.makeDemo(
            referenceDate: referenceDate
        )
        snapshot.missions.append(
            contentsOf: (0..<500).map { index in
                Mission(
                    category: .project,
                    title: "Extended backlog item \(index)",
                    rigidity: .flexible,
                    importance: .normal,
                    urgency: .normal,
                    estimatedDurationMinutes: 30
        ...
        print(
            "PERF_METRIC large_backlog_500_seconds=\(elapsed) "
                + "blocks=\(result.blocks.count) "
                + "unscheduled=\(result.unscheduledMissionIDs.count)"
        )
        XCTAssertLessThan(elapsed, 90)
    }

    func testRepresentativeSnapshotSerializationIsStableAndBounded()
        throws {
        let referenceDate = Date(timeIntervalSince1970: 1_800_000_000)
        var snapshot = MissionControlSeed.makeDemo(
            referenceDate: referenceDate
